...
int main( int argc, char *argv[] ) {
  ...
  if (!data.pipeline || !data.source || !data.audio_convert || !data.audio_sink || !data.video_convert || !data.video_sink) {
...
static void pad_added_handler( GstElement *src, GstPad *new_pad, CustomData *data ) {
  ...
  if ( GST_PAD_LINK_FAILED( ret ) ) {
    ...
    g_print( "Link audio succeded (type '%s').\n", new_pad_type);
  }
  ...
  if ( GST_PAD_LINK_FAILED( ret ) ) {
    ...
    g_print( "Link video succeded (type '%s').\n", new_pad_type);
  }
